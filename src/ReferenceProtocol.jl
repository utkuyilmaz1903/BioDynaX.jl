...
const REFERENCE_PROTOCOL_PRODUCT_BLOCKS = (
...
"""KPI names that decide `reference_protocol_kpis_hold`. Combined F1 is not here."""
...
const REFERENCE_PROTOCOL_EXAMPLE_MUST_CONTAIN = (
    "REFERENCE_PROTOCOL.seed",
    "REFERENCE_PROTOCOL.adam_iterations",
    "REFERENCE_PROTOCOL.bfgs_iterations",
...
const REFERENCE_PROTOCOL_EXAMPLE_MUST_NOT_CONTAIN = (
...
function validate_network_stays_open_source()
    ...
    forbidden = (
        "reference-protocol", "reference_protocol", "unknown_holes", "single-hole",
...
reference_protocol_protocol_kind(; smoke::Bool = false) = smoke ? :smoke : :protocol

function reference_protocol_protocol_n_ics(; smoke::Bool = false)
    smoke ? REFERENCE_PROTOCOL.smoke_n_ics : REFERENCE_PROTOCOL.n_ics
end

function reference_protocol_protocol_n_points(; smoke::Bool = false)
    smoke ? REFERENCE_PROTOCOL.smoke_n_points : REFERENCE_PROTOCOL.n_points
end

"""
    reference_protocol_protocol_ics(; smoke=false)
...
"""
function reference_protocol_protocol_ics(; smoke::Bool = false)
    ics = _unknown_edge_ics()
    n = reference_protocol_protocol_n_ics(; smoke)
...
"""
    reference_protocol_is_protocol(; kwargs...) -> Bool
...
default, so `reference_protocol_is_protocol()` is true and
`reference_protocol_is_protocol(; smoke=true)` is false.
"""
function reference_protocol_is_protocol(;
        smoke::Bool = false,
        seed = REFERENCE_PROTOCOL.seed,
        n_ics = REFERENCE_PROTOCOL.n_ics,
...
"""
    reference_protocol_reproduction(; smoke=false, overrides...)
...
"""
function reference_protocol_reproduction(;
        smoke::Bool = false,
        seed = REFERENCE_PROTOCOL.seed,
        n_ics = reference_protocol_protocol_n_ics(; smoke),
        n_points = reference_protocol_protocol_n_points(; smoke),
        adam_iters = REFERENCE_PROTOCOL.adam_iterations,
        bfgs_iters = smoke ? 0 : REFERENCE_PROTOCOL.bfgs_iterations,
        bootstrap = smoke ? nothing : REFERENCE_PROTOCOL.bootstrap,
        discovery_seed = smoke ? nothing : REFERENCE_PROTOCOL.discovery_seed)
    kind = reference_protocol_protocol_kind(; smoke)
    return (;
        seed,
        n_ics,
        n_points,
        ...
        protocol_kind = kind,
        smoke,
        is_protocol = reference_protocol_is_protocol(;
...
"""
    reference_protocol_training_budget(; smoke=false)
...
"""
function reference_protocol_training_budget(; smoke::Bool = false)
    proto = REFERENCE_PROTOCOL
    repro = reference_protocol_reproduction(; smoke)
    return (;
        seed = proto.seed,
        adam_iterations = proto.adam_iterations,
...
must still fail `reference_protocol_identifiability_holds`.
"""
...
function reference_protocol_identifiability_holds(ident)
    ident === nothing && return false
    hasproperty(ident, :unidentifiable_edge) || return false
    return ident.unidentifiable_edge === true
end

function assert_reference_protocol_identifiability(ident)
    reference_protocol_identifiability_holds(ident) || throw(ErrorException(
...
reference_protocol_residual_holds(residual) = residual ≤ RECOVERY_THRESHOLDS.data_residual

reference_protocol_recall_holds(recall) = recall ≥ RECOVERY_THRESHOLDS.support_recall

function assert_reference_protocol_recall(recall)
    reference_protocol_recall_holds(recall) || throw(ErrorException(
        "true-monomial recall $(recall) is below RECOVERY_THRESHOLDS.support_recall"))
    return recall
end

reference_protocol_f1_meets_skeleton_floor(f1) = f1 ≥ RECOVERY_THRESHOLDS.support_f1_ude

function reference_protocol_f1_reaches_analytical_threshold(f1)
    f1 ≥ RECOVERY_THRESHOLDS.support_f1_clean
end

"""
    reference_protocol_kpi_failures(kpis) -> Vector{Symbol}

Failed acceptance checks, in product order. Combined F1 is never appended.
"""
function reference_protocol_kpi_failures(kpis)
    failures = Symbol[]
    kpis.unidentifiable_edge === true ||
        push!(failures, :unidentifiable_edge)
    reference_protocol_residual_holds(kpis.data_residual) ||
        push!(failures, :data_residual)
    reference_protocol_recall_holds(kpis.support_recall) ||
        push!(failures, :support_recall)
    return failures
end

function assert_reference_protocol_kpis(kpis)
...
"""
    reference_protocol_truth_support(; family=:hill, order=2)

True implicit support used to label leftover monomials. Hill-from-NN is
not opened by calling this.
"""
function reference_protocol_truth_support(; family::Symbol = :hill, order::Int = 2)
...
function reference_protocol_discovery_extras(candidate;
        family::Symbol = :hill, order::Int = 2, atol::Real = 1e-8)
    truth = reference_protocol_truth_support(; family, order)
    return discovered_support_extras(
        candidate, truth.numerator, truth.denominator; atol = atol)
end

function reference_protocol_discovery_extras(discovery::DiscoveryResult; kwargs...)
    discovery.success && !isempty(discovery.candidates) || return String[]
    return reference_protocol_discovery_extras(discovery.candidates[1]; kwargs...)
end

"""
    reference_protocol_f1_attempt_verdict(; extras, reaches_clean) -> Symbol
...
"""
function reference_protocol_f1_attempt_verdict(; extras, reaches_clean::Bool)
    ...
    return :no_extras_but_clean_threshold_not_reached
end
...
function reference_protocol_product_blocks()
    return REFERENCE_PROTOCOL_PRODUCT_BLOCKS
end
...
function format_recovery_protocol(ude;
        ...
        n_points = REFERENCE_PROTOCOL.n_points,
        adam_iters = REFERENCE_PROTOCOL.adam_iterations,
        bfgs_iters = REFERENCE_PROTOCOL.bfgs_iterations,
        bootstrap = REFERENCE_PROTOCOL.bootstrap,
        discovery_seed = REFERENCE_PROTOCOL.discovery_seed,
        smoke::Bool = false,
        equations = nothing)
...
"""
struct ReferenceProtocolFingerprint
    kind::Symbol
    seed::Int
    n_ics::Int
    n_points::Int
...
function reference_protocol_fingerprint(; smoke::Bool = false)
    proto = REFERENCE_PROTOCOL
    if smoke
        return ReferenceProtocolFingerprint(
            :smoke,
            proto.seed,
            proto.smoke_n_ics,
...
function reference_protocol_fingerprint(kind::Symbol)
    kind === :smoke ? reference_protocol_fingerprint(; smoke = true) :
    kind === :protocol ? reference_protocol_fingerprint() :
    throw(ArgumentError("fingerprint kind must be :protocol or :smoke; got $kind"))
end

function reference_protocol_fingerprint_is_protocol(fp::ReferenceProtocolFingerprint)
    proto = REFERENCE_PROTOCOL
    return !fp.smoke &&
           fp.kind === :protocol &&
           fp.seed == proto.seed &&
...
function reference_protocol_fingerprint_is_smoke(fp::ReferenceProtocolFingerprint)
    proto = REFERENCE_PROTOCOL
    return fp.smoke &&
           fp.kind === :smoke &&
           ...
           !reference_protocol_fingerprint_is_protocol(fp)
end

function reference_protocol_fingerprint_holds(fp::ReferenceProtocolFingerprint)
    fp.smoke && return reference_protocol_fingerprint_is_smoke(fp)
    return reference_protocol_fingerprint_is_protocol(fp)
end

function reference_protocol_fingerprint_namedtuple(fp::ReferenceProtocolFingerprint)
    return (;
        kind = fp.kind,
        seed = fp.seed,
        n_ics = fp.n_ics,
        ...
        smoke = fp.smoke,
        is_protocol = reference_protocol_fingerprint_is_protocol(fp))
end

function format_reference_protocol_fingerprint(fp::ReferenceProtocolFingerprint)
    io = IOBuffer()
    ...
    println(io, "  is_protocol: ", reference_protocol_fingerprint_is_protocol(fp))
    return String(take!(io))
end

function format_protocol_result(ident, fingerprint::ReferenceProtocolFingerprint; kwargs...)
    return format_protocol_result(ident;
...
function format_recovery_protocol(ude, fingerprint::ReferenceProtocolFingerprint; kwargs...)
...
function reference_protocol_reproduction(fp::ReferenceProtocolFingerprint)
    return reference_protocol_reproduction(;
        smoke = fp.smoke,
        seed = fp.seed,
        n_ics = fp.n_ics,
        n_points = fp.n_points,
...
function protocol_result_print_order()
    labels = Symbol[]
    for block in REFERENCE_PROTOCOL_PRODUCT_BLOCKS
...
reference_protocol_recovery_admits(model::UDEModel) = count_unknown_destructions(model) == 1

function reference_protocol_recovery_admits(network::BiologicalNetwork)
    count_unknown_destructions(network) == 1
end

function reference_protocol_compiler_stays_open(network::BiologicalNetwork)
...
"""
    assert_reference_protocol_recovery_network(network) -> network
...
"""
function assert_reference_protocol_recovery_network(network::BiologicalNetwork)
...
function reference_protocol_recovery_admission(network::BiologicalNetwork)
    n = count_unknown_destructions(network)
    return (;
        unknown_holes = n,
        validate_open = reference_protocol_compiler_stays_open(network),
        recovery_admits = n == 1,
...
const REFERENCE_PROTOCOL_F1_ATTEMPT = (
    is_protocol = false,
    trains_ude = false,
    n_ics = 0,
...
const REFERENCE_PROTOCOL_F1_ATTEMPT_MUST_CONTAIN = (
    "same library",
    "No new atoms",
    "reference_protocol_discovery_extras",
...
const REFERENCE_PROTOCOL_F1_ATTEMPT_MUST_NOT_CONTAIN = (
    "support_f1_ude = 0.99",
...
function reference_protocol_f1_attempt_row(; extras, f1)
    reaches_clean = reference_protocol_f1_reaches_analytical_threshold(f1)
    meets_skeleton = reference_protocol_f1_meets_skeleton_floor(f1)
    return (;
        extras,
        f1,
        reaches_clean,
        meets_skeleton,
        is_protocol = false,
        verdict = reference_protocol_f1_attempt_verdict(; extras, reaches_clean))
end

function reference_protocol_f1_attempt_spec()
