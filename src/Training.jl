...
trajectory. `mask` (default: all observed) drops missing entries from the
loss; unobserved `NaN` values otherwise produce a non-finite gradient.
Use `train_experiments` for masked multi-replicate data.
"""
...
Fit physical (and optional neural) parameters of a compiled UDE to one
trajectory. `mask` (default: all observed) drops missing entries from the
loss. Use `train_experiments` for masked multi-replicate data.
"""
