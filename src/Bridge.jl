...
candidate, a `DiscoveryRun`, or one of its per-term results) replaces the placeholder of the
...
`regress_unknown_rate`, the regulators in order; for `discover_equations`,
the network's states); a per-term result uses the names of its regulators, and
a `DiscoveryRun` needs `node` unless it has one term.
