@testset "typed biological network" begin
    ...
    @test mechanism.production_terms[1] isa HybridKinetics.InputProductionTerm
    @test mechanism.destruction_terms[1] isa HybridKinetics.NeuralDestructionTerm
end
...
@testset "graph edges derived from unknown reactions" begin
    ...
    reaction_only = HybridKinetics.build_hill_recovery_network(; known = false)
    ...
    known = HybridKinetics.build_hill_recovery_network(; known = true)
    @test candidate_parents(known, 1) == Int[]
    @test HybridKinetics.Graphs.ne(known.graph) == 0
    ...
    self_regulated = HybridKinetics.build_hill_recovery_network(; known = false, parent = 1)
    @test candidate_parents(self_regulated, 1) == [1]
    @test HybridKinetics.Graphs.ne(self_regulated.graph) == 1
    @test candidate_parents(self_regulated, 1) == candidate_parents(
        HybridKinetics._library_study_two_state_graph_network(; parent = 1), 1)
    # Two regulators of one unknown reaction both become parents.
    two = HybridKinetics.build_two_regulator_unknown_network()
    @test candidate_parents(two, 1) == [1, 2]
    # Fixtures that declare their edges explicitly are unchanged.
    four = HybridKinetics.build_three_state_unknown_network()
    @test candidate_parents(four, 1) == [2]
    @test all(isempty(candidate_parents(four, i)) for i in 2:4)
    wrong = HybridKinetics.build_wrong_graph_unknown_network()
