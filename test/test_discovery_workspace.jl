@testset "discovery workspace is not exported" begin
    @test !(:STLSQWorkspace in names(HybridKinetics))
    @test !(:_stlsq_blocked! in names(HybridKinetics))
    @test !(:_fit_implicit_stream in names(HybridKinetics))
    @test !(:evaluate_candidate! in names(HybridKinetics))
    @test !(:each_reusable_library_chunk in names(HybridKinetics))
...
@testset "evaluate_library! writes monomials in place" begin
    ...
    HybridKinetics.evaluate_library!(out, spec.numerator, X)
    ptr = pointer(out)
    HybridKinetics.evaluate_library!(out, spec.numerator, X)
    @test pointer(out) === ptr
    allocating = HybridKinetics.evaluate_library(spec.numerator, X)
    @test out ≈ allocating
    col = out[:, 2]
    col_ptr = pointer(col)
    HybridKinetics.evaluate_term!(col, spec.numerator[2], X)
...
@testset "blocked STLSQ workspace matches the allocating path" begin
    ...
    report = HybridKinetics.stlsq_path_agreement(A, y, 5e-2; chunk_size = 32)
    @test report.holds
...
@testset "STLSQ workspace buffers are stable across calls" begin
    ...
    ws = HybridKinetics.allocate_stlsq_workspace(Float64, 120, 6, 24)
    gram_ptr = pointer(ws.gram)
    chunk_ptr = pointer(ws.design_chunk)
    first = collect(HybridKinetics._stlsq_blocked!(ws, A, y, 1e-2))
    @test pointer(ws.gram) === gram_ptr
    @test pointer(ws.design_chunk) === chunk_ptr
    second = collect(HybridKinetics._stlsq_blocked!(ws, A, y, 1e-2))
    @test first ≈ second
    @test pointer(ws.gram) === gram_ptr
    @test ws.resize_count == 1
    via_api = HybridKinetics._stlsq_blocked(A, y, 1e-2; chunk_size = 24, workspace = ws)
    @test via_api ≈ first
end
...
@testset "workspace STLSQ allocates less than a fresh blocked call" begin
    ...
    report = HybridKinetics.discovery_workspace_alloc_report(A, y, 2e-2; chunk_size = 40)
...
@testset "chunk_size one and larger than n agree" begin
    ...
    a = HybridKinetics._stlsq_blocked(A, y, 1e-2; chunk_size = 1)
    b = HybridKinetics._stlsq_blocked(A, y, 1e-2; chunk_size = 40)
    c = HybridKinetics._stlsq_blocked(A, y, 1e-2; chunk_size = 256)
    @test a ≈ b
    @test b ≈ c
end
...
@testset "implicit stream agrees with materialised design" begin
    ...
    report = HybridKinetics.implicit_stream_agreement(
    ...
    pred, denvals = HybridKinetics._evaluate_candidate(
...
@testset "Hill recovery still holds on the streamed implicit path" begin
    ...
    pred, denvals = HybridKinetics._evaluate_candidate(spec, num, den, X)
    @test mean(abs2, pred .- hill) < 1e-7
    @test minimum(abs, denvals) > 0
    streamed = HybridKinetics._fit_implicit_stream(
        spec, X, hill, collect(eachindex(x)), 1e-7; chunk_size = 28)
    @test streamed[1] ≈ num
    @test streamed[2] ≈ den
end
...
@testset "evaluate_candidate! matches _evaluate_candidate" begin
    ...
    report = HybridKinetics.evaluate_candidate_agreement(spec, num, den, X)
    @test report.holds
end
...
@testset "backend chunk_size helper is consistent" begin
    ...
    @test HybridKinetics._backend_chunk_size(implicit) == 48
    @test HybridKinetics._backend_chunk_size(ExplicitSTLSQ()) == 256
    @test HybridKinetics._backend_chunk_size(DataDrivenSparseSTLSQ()) == 256
    cfg = DiscoveryConfig(backend = ImplicitSINDyPI(chunk_size = 16))
    @test HybridKinetics._backend_chunk_size(cfg) == 16
end
...
@testset "bootstrap reuses one streaming workspace" begin
    ...
    spec = local_basis(
        ...
        X, derivative = D)
    report = HybridKinetics.bootstrap_workspace_reuse_report(
...
@testset "masked implicit refit reuses the implicit workspace" begin
    ...
    ws = HybridKinetics.allocate_implicit_workspace(Float64, 80, n_num, n_den, 32)
    keep_n = trues(n_num)
    keep_d = trues(n_den)
    a = HybridKinetics._refit_masked_implicit(
        spec, X, y, keep_n, keep_d, 1e-6; workspace = ws)
    start = ws.resize_count
    b = HybridKinetics._refit_masked_implicit(
        spec, X, y, keep_n, keep_d, 1e-6; workspace = ws)
    @test a[1] ≈ b[1]
    @test a[2] ≈ b[2]
...
@testset "explicit discovery uses the backend chunk helper" begin
    src = read(HybridKinetics.discovery_jl_source_path(), String)
...
@testset "streaming interface source and docs hold" begin
    @test HybridKinetics.discovery_jl_uses_workspace()
    @test HybridKinetics.basis_factory_evaluates_in_place()
end
...
@testset "0/2-hole networks stay open on validate_network after workspace include" begin
    ...
    two = HybridKinetics.build_dual_unknown_network()
